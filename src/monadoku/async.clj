...
(defn debug [& more]  #_(>!! *printer-chan* [println more]))
...
(defn box-for-cell [cell]
  (+ (* 3 (quot cell 27)) (quot (mod cell 9) 3)))
...
(defn make-grid []
  (let [name [::Grid 0]
        ...
        boxes (vec (map #(make-container ::Box %1 publisher publication %2) (range 9) (->> (partition 3 cells)
                                                                                           (partition 3)
                                                                                           (apply interleave)
                                                                                           (partition 3)
                                                                                           (map flatten))))
...
(defmethod is-value ::Cell [{:keys [name containers grid publisher] :as cell} {val :val}]
...
(defmethod is-not-value ::Cell [{:keys [name containers possibilities publisher] :as cell} {:keys [val sender]}]
  (let [remaining (disj possibilities val)]
    ;(debug name "has been told is-not-value" val remaining (:value cell) "by" sender containers)
    (tell-all! publisher containers name is-not-value val)
...
(defn remove-possibility-for-value [possibilities cell val {:keys [name publisher]}]
  (let [updated (remove #(= cell %) possibilities)]
    ...
    (when (= (count updated) 1)
      ...
      (tell! publisher (first updated) is-value val nil))
    updated))
...
(defmethod is-value ::Grid [grid {:keys [val sender]}]
  (debug (:name grid) "was told is-value" sender val (:count grid))
...
(defn do-puzzle [puzzle name]
  (let [start (System/currentTimeMillis)
    ...
    (alt!!
      (timeout 5000) ([v c] (log name "Timed out" c v))
      result ([r _]
